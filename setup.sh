...
if mkdir -p "$kitty_config_dir" && cat << 'EOF' >> "$kitty_config_dir/kitty.conf"
...
if curl -fsSL https://raw.githubusercontent.com/LazyVim/starter/main/scripts/setup.sh | bash; then
    echo "LazyVim installed successfully"
else
    echo "Failed to install LazyVim"
fi
